const fs = require('fs');
const https = require('https');
const zlib = require('zlib');

const url = "https://storage.googleapis.com/eas-workflows-production/logs/3ab87f13-3f1d-487a-8ff7-0374f04f7aa6/17ffb343-d4a5-449d-9c98-f32cf4c7faa6/2026-06-23T14%3A24%3A20Z-c93e55fb-8aa8-4594-9602-7d372824ec68.txt?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=www-production%40exponentjs.iam.gserviceaccount.com%2F20260623%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20260623T142539Z&X-Goog-Expires=900&X-Goog-SignedHeaders=host&X-Goog-Signature=";

https.get(url, (res) => {
  console.log('Status code:', res.statusCode);
  console.log('Headers:', res.headers);

  const chunks = [];
  res.on('data', (chunk) => {
    chunks.push(chunk);
  });

  res.on('end', () => {
    const buffer = Buffer.concat(chunks);
    const encoding = res.headers['content-encoding'];
    
    if (encoding === 'br') {
      console.log('Decompressing Brotli...');
      zlib.brotliDecompress(buffer, (err, decompressed) => {
        if (err) {
          console.error('Brotli decompress failed:', err);
          fs.writeFileSync('decompressed_log.txt', buffer);
          console.log('Saved raw (compressed) to decompressed_log.txt');
        } else {
          fs.writeFileSync('decompressed_log.txt', decompressed.toString('utf8'));
          console.log('Decompressed successfully and saved to decompressed_log.txt');
        }
      });
    } else if (encoding === 'gzip') {
      console.log('Decompressing Gzip...');
      zlib.gunzip(buffer, (err, decompressed) => {
        if (err) {
          console.error('Gzip decompress failed:', err);
          fs.writeFileSync('decompressed_log.txt', buffer);
        } else {
          fs.writeFileSync('decompressed_log.txt', decompressed.toString('utf8'));
          console.log('Decompressed successfully and saved to decompressed_log.txt');
        }
      });
    } else {
      console.log('No encoding / already uncompressed');
      fs.writeFileSync('decompressed_log.txt', buffer.toString('utf8'));
      console.log('Saved to decompressed_log.txt');
    }
  });
}).on('error', (err) => {
  console.error('Request failed:', err);
});
